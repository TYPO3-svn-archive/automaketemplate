...
class tx_automaketemplate_pi1 extends \TYPO3\CMS\Frontend\Plugin\AbstractPlugin {
	...
	// Same as class name
	public $prefixId = 'tx_automaketemplate_pi1';
	// Path to this script relative to the extension dir.
	public $scriptRelPath = 'pi1/class.tx_automaketemplate_pi1.php';
	// The extension key.
	...
	protected function recursiveBlockSplitting($content) {
		...
		foreach ($parts as $blockNumber => $blockContent) {
			if ($blockNumber % 2) {
				...
				// The first tag's content
				$firstTag = $this->htmlParse->getFirstTag($blockContent);
				// The 'name' of the first tag
				$firstTagName = $this->htmlParse->getFirstTagName($blockContent);
				$endTag = '</' . strtolower($firstTagName) . '>';
				// Finally remove the first tag (unless we do this, the recursivity will be eternal!
				$blockContent = $this->htmlParse->removeFirstAndLastTag($blockContent);
